...
static void validate_ignore(zend_attribute *attr, uint32_t target,
  ...
  if (scope->ce_flags & ZEND_ACC_INTERFACE) {
    zend_error_noreturn(E_ERROR,
                        "Cannot apply #[Normalizer\\Ignore] to interface");
  }
  if (scope->ce_flags & ZEND_ACC_READONLY_CLASS) {
    zend_error_noreturn(
        E_ERROR, "Cannot apply #[Normalizer\\Ignore] to readonly class %s",
        ZSTR_VAL(scope->name));
  }
}
...
void php_register_ignore_attribute() {
  ...
  zend_function_entry methods[] = {PHP_ME(Ignore, __construct,
                                          arginfo_ignore_attribute_construct,
  ...
  zend_attribute *attribute_Attribute_class_Ignore_0 = zend_add_class_attribute(
      class_entry, attribute_name_Attribute_class_Ignore_0, 1);
  zend_string_release(attribute_name_Attribute_class_Ignore_0);
  zval attribute_Attribute_class_Ignore_0_arg0;
  ZVAL_LONG(&attribute_Attribute_class_Ignore_0_arg0,
            ZEND_ATTRIBUTE_TARGET_PROPERTY | ZEND_ATTRIBUTE_TARGET_METHOD);
  ZVAL_COPY_VALUE(&attribute_Attribute_class_Ignore_0->args[0].value,
                  &attribute_Attribute_class_Ignore_0_arg0);
